      ...
      case "ErrBounce":
        return (
          <div style={{ fontSize: "14px" }}>
            {"Bounced with " + helpers.provName(this.props.variant, parts[1])}
          </div>
        );
    ...
    (this.props.corroboration.Properties.Inconsistencies || []).forEach(
      (inconsistency) => {
        if (inconsistency.Province != "") {
          ...
          provInconsistencies[inconsistency.Province].push(inconsistency);
        } else {
      ...
      orders: (this.props.corroboration.Properties.Orders || []).reduce(
        (sum, el) => {
          ...
          if (provInconsistencies[el.Parts[0]]) {
            el.Inconsistencies = provInconsistencies[el.Parts[0]].reduce(
              (sum, el) => {
      ...
      this.setState({
        resolutions:
          ...
          this.props.phase.Properties.Resolutions instanceof Array
            ? this.props.phase.Properties.Resolutions.reduce((sum, el) => {
                sum[el.Province] = el;
                return sum;
              }, {})
            : {},
        ...
        <div style={{ maxWidth: "960px", margin: "auto" }}>
          <MaterialUI.List>
            {this.props.variant.Properties.Nations.slice()
              .sort((n1, n2) => {
                if (this.props.member && n1 == this.props.member.Nation) {
                  return -1;
                } else if (
                  this.props.member &&
              ...
              .map((nation) => {
                ...
                const SCs = (this.props.phase.Properties.SCs || []).filter(
                  (sc) => {
                ...
                border: 1px solid ` + helpers.natCol(nation, this.props.variant)
                        ...
                        />
                        <span
                          style={{ lineHeight: "1.2em", marginLeft: "8px" }}
                        >
                      ...
                      <MaterialUI.List>
                        {(this.state.orders[nation] || []).map((order) => {
                          return (
                                ...
                                {(order.Inconsistencies || []).map((incon) => {
                                  return (
                                    <div
                                      ...
                                      key={incon}
                                    >
                                      {this.presentInconsistency(incon)}
                                    </div>
                                  );
                                })}
                              </MaterialUI.ListItemText>
                ...
                font-size: 14px;
                    `)}
                        ...
                        {this.props.member && this.props.member.Nation == nation
                          ? this.state.homelessInconsistencies.map((incon) => {
                              if (incon.Province) {
                                return (
                                  <MaterialUI.ListItem
                                    key={incon.Province}
          ...
          >
            ...
            >
              <MaterialUI.Checkbox
                ...
                disabled={
                  this.state.phaseStates[this.props.member.Nation].Properties
                    .NoOrders
                }
                checked={
                  this.state.phaseStates[this.props.member.Nation].Properties
