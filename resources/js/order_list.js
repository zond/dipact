...
export default class OrderList extends React.Component {
        ...
        .then((js) => {
          ...
          this.setState(
            (state, props) => {
              ...
              state.phaseStates[nation].Properties = js.Properties;
              return state;
            },
            (_) => {
      ...
      loadPromise.then((js) => {
        ...
        this.setState((state, props) => {
          ...
          js.Properties.forEach((phaseState) => {
            state.phaseStates[phaseState.Properties.Nation] = phaseState;
          });
          return state;
        });
      });
  ...
  render() {
      ...
      <div>
        <MaterialUI.List>
          {this.props.variant.Properties.Nations.map((nation) => {
            ...
            if (phaseState || this.props.orders[nation]) {
              ...
              {/*TODO: make sure the first element is the current players nation (if currently logged in as a player of this game)*/}
                  <ul style={{ paddingLeft: "0px" }}>
                    ...
                    >
                      ...
                      <div style={{ alignSelf: "center" }}>
                        ...
                        {/*TODO: If this is the current player AND it's a current phase (e.g. action is possible), don't show this tooltip + content (it will use the "checkboxbutton"). So, also for nation == currentplayer but older phases, use this icon instead of the button.*/}
                        {/*TODO: In all these, first check if there is a phaseState property (I don't understand the phaseState lifecycle.*/}
                      
                        <MaterialUI.Tooltip
                          className={helpers.scopedClass(
                            phaseState.Properties.WantsDIAS
                              ? ""
                              : "display: none;"
                          )}
                          title="Wants a draw"
                        ...
                        <MaterialUI.Tooltip
                          className={helpers.scopedClass(
                            phaseState.Properties.OnProbation
                              ? ""
                              : "display: none;"
                          )}
                        ...
                        >
                          <MaterialUI.SvgIcon
                            color="primary"
                            style={{ paddingLeft: "8px", color: "#b71c1c" }}
                          >
                      ...
                      {/*TODO: If nation is the current player AND is latest phase (action is possible), show this button, otherwise hide */}
                      <div style={{ alignSelf: "center", marginLeft: "auto" }}>
                        ...
                        >
                          <MaterialUI.Checkbox
                            checked={phaseState.Properties.WantsDIAS}
                            disabled={
                              !phaseState.Links ||
                          ...
                          />
                          Accept a draw
                          <MaterialUI.SvgIcon style={{ paddingLeft: "8px" }}>
                            <path d=      { 
                              phaseState
                                .Properties
                                .WantsDIAS
                            ? "M2.98188996,2.24133335 L3.88833335,3.148 L3.8,3.23743687 L20.7705627,20.2079996 L20.8593333,20.119 L21.3666663,20.6261097 L20.0261097,21.9666663 L14.4292636,16.3704135 C14.0775047,16.5664056 13.6995541,16.7212717 13.301866,16.8285576 L13,16.9 L13,19.08 C15.489899,19.4617845 15.9132657,21.2212572 15.9852522,21.8085585 L16,22 L8,22 L8.00876781,21.8621764 C8.05962111,21.354459 8.40542355,19.5936066 10.7568801,19.1228674 L11,19.08 L11,16.9 C9.11538462,16.5153846 7.61908284,15.0767751 7.15117205,13.224249 L7.1,13 L4,13 C2.95,13 2.0822314,12.1799587 2.00551277,11.1486946 L2,11 L2,4 L2.06033335,4 L1.64133335,3.58188996 L2.98188996,2.24133335 Z M17,2 L17,4 L22,4 L22,11 C22,12.05 21.1799587,12.9177686 20.1486946,12.9944872 L20,13 L16.9,13 C16.852859,13.2309911 16.7898842,13.4561487 16.7122542,13.6742943 L6.99933335,3.962 L7,2 L17,2 Z M4.06033335,6 L4,6 L4,11 L7,11 L6.99933335,8.939 L4.06033335,6 Z M20,6 L17,6 L17,11 L20,11 L20,6 Z" : "M17 4V2H7V4H2V11C2 12.1 2.9 13 4 13H7.1C7.5 14.96 9.04 16.5 11 16.9V19.08C8 19.54 8 22 8 22H16C16 22 16 19.54 13 19.08V16.9C14.96 16.5 16.5 14.96 16.9 13H20C21.1 13 22 12.1 22 11V4H17M4 11V6H7V11L4 11M20 11L17 11V6H20L20 11Z"}></path>
                          </MaterialUI.SvgIcon>
                        </MaterialUI.Button>
                      </div>
                    </MaterialUI.ListSubheader>
                    ...
                    <MaterialUI.List>
                      {(this.props.orders[nation] || []).map((order) => {
                        return (
                          <MaterialUI.ListItem key={order.Name}>
                  ...
                  {/* TODO: I think the groups below can be removed. I don't understand the disabled statement here, but I think it can be used with display:hidden in the elements above*/}
                    {phaseState ? (
                      <MaterialUI.FormGroup>
                        <MaterialUI.FormControlLabel
                          ...
                          control={
                            <MaterialUI.Checkbox
                              checked={phaseState.Properties.ReadyToResolve}
                              disabled={
                                !phaseState.Links ||
                                !phaseState.Links.find((l) => {
                                  return l.Rel == "update";
                              ...
                              onChange={this.toggleReady(nation)}
                            />
                          }
                        ...
                        <MaterialUI.FormControlLabel
                          ...
                          control={
                            <MaterialUI.Checkbox
                              ...
                              onChange={this.toggleDIAS(nation)}
                            />
                          }
                          label="Wants draw"
                        />
        ...
        <div
          ...
          className={helpers.scopedClass(`
					...
					`)}
        />
          {/*TODO: If observer or currentplayer but not in current phase, hide the whole app bar (as orders can't be given).*/}
    ...
    `)}
        >
          {/*TODO: below, the onClick rather than the checkbox changestate should handle the toggle ready, otherwise you click the checkbox (which both counts as button onclick and checkbox switchstate and it "reselects" itself in one click*/}
          <MaterialUI.Button
            color="secondary"
            variant="contained"
          ...
          >
          {/*TODO: Similar to the "checkboxbutton" on top, the checkbox should follow the phasestate.properties.ready... elements. However, since we're not in the list loop, I don't know how to get the current player (because these should only refer to the current player*/}
            <MaterialUI.Checkbox
              style={{ padding: "0px 8px 0px 0px" }}
              color="primary"
            />
