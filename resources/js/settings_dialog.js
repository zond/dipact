...
export default class SettingsDialog extends React.Component {
  ...
  addColorOverride() {
    ...
    this.setState((state, props) => {
      ...
      state.userConfig.Properties.Colors = Globals.colorOverrides.positions;
      Object.keys(Globals.colorOverrides.nations || {}).forEach((nation) => {
      ...
      Object.keys(Globals.colorOverrides.variants || {}).forEach((variant) => {
        Object.keys(Globals.colorOverrides.variants[variant] || {}).forEach(
          (nation) => {
            state.userConfig.Properties.Colors.push(
  ...
  saveConfig() {
    this.state.userConfig.Properties.PhaseDeadlineWarningMinutesAhead = Number.parseInt(
      this.state.userConfig.Properties.PhaseDeadlineWarningMinutesAhead || "0"
    );
        ...
        {this.state.userConfig ? (
          <React.Fragment>
            ...
            >
              <MaterialUI.Typography
                variant="subtitle2"
                style={{ color: "rgba(40, 26, 26, 0.56)", padding: "16px 0px" }}
              >
                Notifications
              </MaterialUI.Typography>
              <div width="100%">
                <MaterialUI.FormControlLabel
                  ...
                  classes={{ root: helpers.scopedClass("padding-left:0px") }}
                  control={
                    <MaterialUI.Switch
                      ...
                      onChange={(ev) => {
                        ...
                        Globals.messaging.start().then((js) => {
                          helpers.decProgress();
                          let currentConfig = this.state.userConfig;
                          if (js) {
                            currentConfig = js;
                          }
                          this.setState(
              ...
              <MaterialUI.FormControlLabel
                control={
                  <MaterialUI.Switch
                    checked={
                      this.state.userConfig.Properties.MailConfig.Enabled
                    }
                    onChange={(ev) => {
                      ev.persist();
                      this.setState((state, props) => {
              ...
              <MaterialUI.TextField
                inputProps={{ min: 0 }}
                fullWidth
                style={{ marginTop: "1px", maxWidth: "180px" }}
                disabled={
                  !this.state.userConfig.Properties.MailConfig.Enabled &&
                  !Globals.messaging.tokenEnabled
                }
              ...
              <MaterialUI.Typography variant="body2" style={{marginBottom: "8px"}}>
                Here you can override the colours of each nation to fit your o.
              </MaterialUI.Typography> */}
              ...
              <MaterialUI.Select
                ...
                className={helpers.scopedClass("flex-grow: 0;")}
                onChange={(ev) => {
                  const variant = Globals.variants.find((v) => {
                    return (
                      v.Properties.Name == this.state.newColorOverrideVariant
                    );
                  });
                  let nation = this.state.newColorOverrideNation;
                  if (
                    ev.target.value == "variant" &&
                    variant.Properties.Nations.indexOf(nation) == -1
                  ) {
                    nation = variant.Properties.Nations[0];
                  }
                  this.setState({
                    newColorOverrideType: ev.target.value,
              ...
              >
                ...
                <MaterialUI.MenuItem value="nation">Nation</MaterialUI.MenuItem>
...
*/}
                <MaterialUI.FormControl style={{ marginBottom: "8px" }}>
                  <MaterialUI.InputLabel shrink id="variantinputlabel">
                    Variant
...
{console.log(Globals)}
                <div style={{ display: "flex", flexDirection: "column", marginBottom: "200px" }}>
                  {Globals.variants
                    ...
                    .Properties.Nations.map((nation, index) => {
                      return (
                        <div style={{display: "flex", height: "48px", alignItems: "center"}} key={nation} value={nation}>
                          <MaterialUI.Typography>
                            {nation} 
                          </MaterialUI.Typography>
                     {/*     <div style={{backgroundColor: Globals.contrastColors[index], marginLeft: "auto", padding: "6px 4px", borderRadius: "6px"}} >
                                                      {Globals.contrastColors[index]}
                        ...
                        */}
                        ...
                        className={helpers.scopedClass("flex-grow: 0; margin-right: 4px;")}
                        value={Globals.contrastColors[index]}
                        onSelect={(col) => {
                          this.setState({
              ...
              {helpers.createIcon("\ue3c9") /* edit icon */}
                ...
                <MaterialUI.Select
                  value={this.state.newColorOverrideNation}
                  className={helpers.scopedClass("flex-grow: 1;")}
                  onChange={(ev) => {
                    this.setState({
                      newColorOverrideNation: ev.target.value,
                    });
                  }}
                >
                  {(this.state.newColorOverrideType == "nation"
                    ? this.allNations
                    ...
                    return (
                      <MaterialUI.MenuItem key={nation} value={nation}>
                        {nation} 
                      </MaterialUI.MenuItem>
                    );
                  })}
              ...
              {/* TODO: applied overrides list */}

              <MaterialUI.List>
                {((_) => {
                  let overridePos = 0;
                  return (this.state.userConfig.Properties.Colors || []).map(
                    (color, idx) => {
                      if (color == "") {
                        return "";
                      }
                      ...
                      if (parts.length == 1) {
                        return (
                          <MaterialUI.ListItem
                            button
                            key={"override_" + overridePos + "_" + color}
                          >
                            <div
                              className={helpers.scopedClass("flex: 1 1 auto;")}
                            >
                              Override{" "}
                              <span
                                style={{
                                  backgroundColor:
                                    Globals.contrastColors[overridePos],
                                }}
                              >
                                position
                                {" " + overridePos++}
                            ...
                            <Color
                              value={color}
                              onSelect={this.newColorSetter(idx, "")}
                            />
                            <MaterialUI.ListItemSecondaryAction>
                              <MaterialUI.IconButton
                                edge="end"
                                onClick={this.newColorDeleter(idx)}
                              >
                        ...
                        return (
                          <MaterialUI.ListItem
                            button
                            key={"nation_" + parts[0] + "_" + parts[1]}
                          >
                            <MaterialUI.ListItemText
                              primary={
                                "Override " +
                                Globals.colorOverrides.nationCodes[parts[0]]
                              }
                            />
                            <Color
                              value={parts[1]}
                        ...
                        return (
                          ...
                          >
                            <MaterialUI.ListItemText
                              primary={
                                "Override " +
                                Globals.colorOverrides.nationCodes[parts[1]] +
                                " in " +
                                Globals.colorOverrides.variantCodes[parts[0]]
                              }
                            />
                            <Color
                              value={parts[2]}
