...
export default class PreliminaryScores extends React.Component {
  ...
  componentDidUpdate(prevProps, prevState, snapshot) {
    ...
    if (this.state.open && this.state.canvas && !this.state.chart) {
      ...
      const datasets = this.props.variant.Properties.Nations.map((nation) => {
        ...
        return {
          ...
          data: adjustmentPhases.map((phase) => {
            const score = phase.Properties.PreliminaryScores.find((score) => {
            ...
            tooltips: {
              callbacks: {
                footer: (tooltipItems, data) => {
                  return tooltipItems
                    .map((item) => {
                      const nation = this.props.variant.Properties.Nations[
                        item.datasetIndex
                      ];
                      const score = adjustmentPhases[
                        item.index
                      ].Properties.PreliminaryScores.find((score) => {
                        return score.Member == nation;
                      });
                      return (
                        nation +
      ...
      >
        ...
        <MaterialUI.DialogContent>
          <MaterialUI.Typography>
            If the game ends in a draw, points will be divided as below (based
            on the{" "}
