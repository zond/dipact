...
class Messaging {
  ...
  findGlobalToken() {
    if (!this.userConfig.Properties.FCMTokens) {
      this.userConfig.Properties.FCMTokens = [];
    }
    return this.userConfig.Properties.FCMTokens.find((t) => {
      return t.App == this.tokenApp;
    });
  }
  setGlobalToken(newToken) {
    if (!this.userConfig.Properties.FCMTokens) {
      this.userConfig.Properties.FCMTokens = [newToken];
      return;
    }
    let foundToken = false;
    this.userConfig.Properties.FCMTokens =
      this.userConfig.Properties.FCMTokens.map((t) => {
    ...
    if (!foundToken) {
      this.userConfig.Properties.FCMTokens.push(newToken);
    }
  }
  uploadToken() {
    return new Promise((res) => {
      ...
      if (foundToken) {
        ...
        updateServer = !isEqual(wantedToken, foundToken);
      } else {
        this.userConfig.Properties.PhaseDeadlineWarningMinutesAhead = 60;
        updateServer = true;
      }
      // TODO(zond): Remove the cleaning stuff here after 2020-08-01.
      // Clean up the old style FCM tokens.
      this.userConfig.Properties.FCMTokens =
        this.userConfig.Properties.FCMTokens.filter((t) => {
      ...
      if (updateServer) {
        ...
        // let updateLink = this.userConfig.Links.find((l) => {
        //   return l.Rel == "update";
        // });
        // return helpers TODO
        //   .safeFetch(
        ...
        //       body: JSON.stringify(this.userConfig.Properties),
        //       headers: {
        ...
        //   .then((js) => {
        //     this.userConfig = js;
        //     // helpers.parseUserConfigColors(); TODO replace
        ...
        //       console.log("Saved token on server to enable FCM push messages.");
        //     }
  ...
  handle(payload) {
    ...
    if (this.subscribers[payload.data.type]) {
      for (let key in this.subscribers[payload.data.type]) {
        handled = handled || this.subscribers[payload.data.type][key](payload);
      }
    }
    return handled;
  }
