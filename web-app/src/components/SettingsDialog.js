...
class SettingsDialog extends React.Component {
	...
	newColorDeleter(nation) {
		return (_) => {
			if (Globals.colorOverrides.variants[this.state.newColorOverrideVariant]) {
	...
	componentDidUpdate(prevProps, prevState, snapshot) {
		...
		if (colourNonSCs !== null && colourNonSCs.length && this.state.colourNonSCs.toString() !== colourNonSCs) {
					...
					<React.Fragment>
							...
							<div className={classes.container}>
											...
											(window.Wrapper && window.Wrapper.startFCM) ? (
												Globals.messaging.tokenOnServer ? (
													...
													window.Wrapper.notificationStatus ? (
														window.Wrapper.notificationStatus() !== "OK" ? (
															window.Wrapper.notificationStatus()
														) : (
															""
														)
								...
								<div>
									...
									>
										...
										ownership of the supply centers around it.
									</Typography>
								</div>
								<Typography variant="subtitle2" className={classes.subtitle2}>
													...
													>
														...
														>
															<Color
																...
																onSelect={this.newColorSetter(nation)}
															/>
														</div>
														...
														Globals.contrastColors[index] !==
															...
															<div
																onClick={this.newColorDeleter(nation)}
																style={{
																	color: "#281A1A",
																}}
															>
						...
						>
											...
											onClick={(ev) => {
												this.setState({
													resetSettingsChecked:
														!this.state.resetSettingsChecked,
												});
											}}
							...
							<Button
								...
								onClick={(_) => {
									if (document.getElementById("sure-about-reset").checked) {
										this.setState(
											(state, props) => {
												state = Object.assign({}, state);
